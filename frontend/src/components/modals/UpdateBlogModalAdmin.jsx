...
function UpdateBlogModalAdmin() {
  ...
  const { blogTitle, author, blogBody, publish, suspended, featured, country } =
    formData
  ...
  useEffect(() => {
    // need || to stop controllded input error
  ...
  const onSubmit = (e) => {
    ...
    // update the UI
    const data = adminBlogs.map((item) =>
      item._id === blogID ? updatedData : item
    )
