...
function Register() {
  ...
  // Handling input change with auto-focus (auto moves to the next field)
  const onChange = (e) => {
    ...
    if (name === 'day' || name === 'month') {
      ...
      // ensure number and auto set if max is exceded
  ...
  const onSubmit = (e) => {
    e.preventDefault()
    // make sure tier is selcted (prem / free)
    if (tier === '' || tier === 'choose') {
